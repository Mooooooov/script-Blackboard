...
async function start() {
    ...
    // Ir para a página 
    await page.goto('https://iesb.blackboard.com/ultra/courses/_541803_1/outline', {
        waitUntil: 'load',
        timeout: 0,
    });
    ...
    // Fechar o aviso, caso tenha
    const caixaAviso = 'button[class*="close-modal-button"]'
    if (await page.$(caixaAviso) !== null) {
        await page.click(caixaAviso)
    ...
    const listaAlunos = document.querySelector('.participant-details')
    
    
    listaAlunos.addEventListener(() => {
        if (listaAlunos != 'JÚLIA GOMES'){
            page.close()
        }
    })
